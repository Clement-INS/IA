...
% La hauteur de lavl A est d�finie par :
...
% put_flat(+Avl)                    O(N)			
% put_90(+Avl)                      O(N)			
% belongs(+Elem, +Avl)              O(log N)		% appartenance dun noeud � Q
...
insert(Elem, AVL, NEW_AVL) :-
	...
	(Elem = Racine ->
			% l�l�ment est d�j� present, pas dinsertion possible
		fail
	;
		(Elem @< Racine ->
	...
	%------------------------------------------------
	% On suppose que l�l�ment � supprimer appartient bien � lAVL,
	% sinon le predicat �choue (en particulier si lAVL est vide).
